<?php /**
		 ...
		 */ class iam_backup
	{
		...
		function _get_browser_type()
		{
            ...
            if ( preg_match('/Opera/i', strtoupper($_SERVER["HTTP_USER_AGENT"]), $log_version) )
			{
				$USER_BROWSER_AGENT = 'OPERA';
			}
			else
				//if ( ereg('MSIE ([0-9].[0-9]{1,2})', strtoupper($_SERVER["HTTP_USER_AGENT"]), $log_version) )
                if ( preg_match('/MSIE/i', strtoupper($_SERVER["HTTP_USER_AGENT"]), $log_version) )
				{
					$USER_BROWSER_AGENT = 'IE';
				}
				else
					//if ( ereg('OMNIWEB/([0-9].[0-9]{1,2})', strtoupper($_SERVER["HTTP_USER_AGENT"]), $log_version) )
                    if ( preg_match('/OMNIWEB/i', strtoupper($_SERVER["HTTP_USER_AGENT"]), $log_version) )
					{
						$USER_BROWSER_AGENT = 'OMNIWEB';
					}
					else
						//if ( ereg('MOZILLA/([0-9].[0-9]{1,2})', strtoupper($_SERVER["HTTP_USER_AGENT"]), $log_version) )
                        if ( preg_match('/MOZILLA/i', strtoupper($_SERVER["HTTP_USER_AGENT"]), $log_version) )
						{
							$USER_BROWSER_AGENT = 'MOZILLA';
						}
						else
							//if ( ereg('KONQUEROR/([0-9].[0-9]{1,2})', strtoupper($_SERVER["HTTP_USER_AGENT"]), $log_version) )
                            if ( preg_match('/Firefox/i', strtoupper($_SERVER["HTTP_USER_AGENT"]), $log_version) )
							{
